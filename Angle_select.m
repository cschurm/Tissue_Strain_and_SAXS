...
hold on
for i = 1:length(valid_strain)
     plot(valid_strain(i),Angle(i),'r+');
     % Label the points with the index
     text(valid_strain(i),Angle(i)+lbl_dwn,num2str(i));
end
title('Indices','FontWeight','bold','FontSize',16)

subplot(3,1,3)
plot(valid_strain,Angle,'b*')
title('Select points / ROI for saving (click and drag to select points)','FontWeight','bold','FontSize',16)
[~, Angle_strain, Angle_saved]=selectdata;
hold off
Angle_strain = Angle_strain';
Angle_saved = Angle_saved';

figure
plot (Angle_strain,Angle_saved,'b*')
