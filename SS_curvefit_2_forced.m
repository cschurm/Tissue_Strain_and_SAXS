function SS_curvefit_2_forced(re_strain, re_stress)
%%load
...
p1 = polyfix(re_strain,re_stress,2,re_strain(end),re_stress(end));
...
p2 = polyfix(fit_strain,re_stress,2,fit_strain(end),re_stress(end));

%% Generate points from fit
a = p2(1); b = p2(2); c = p2(3);
n=1;
for ii = 0:0.02:round(fit_strain(end),2)
    false_strain(n)=ii;
    false_stress(n)=a*ii^2+b*ii+c;
    n=n+1;
end
%% Find r^2 of new fit
%grand mean (y_bar)
...
for ii = 1:length(re_strain)
    if fit_strain(ii) > 0
        y_res_vec(ii)=(a*fit_strain(ii)^2+b*fit_strain(ii)+c-re_stress(ii))^2;
        y_tot_vec(ii)=(a*fit_strain(ii)^2+b*fit_strain(ii)+c-y_bar)^2;
    end
end
y_res=sum(y_res_vec);
y_tot=sum(y_tot_vec);
...
%% Plot fit with data.
% Plot fit with data
figure( 'Name', 'Fit Curves - Poly2 Forced' );
